...
// Kinds cover additive, metadata, and destructive actions.
type Kind string
...
func Compute(current []model.Collection, desired model.Desired, opts ...Options) Plan {
	...
	var steps []Step
	// additive (create, add field, add index, update rules)
	...
	if cfg.AllowDestructive {
		for _, cur := range current {
			want, ok := desByName[cur.Name]
			if !ok { steps = append(steps, Step{Kind: DropCollection, Msg: "Drop collection "+cur.Name, DropCollection: struct{ Name string }{cur.Name}}); continue }
