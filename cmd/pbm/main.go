...
func main(){
	...
	case "plan", "apply":
		var pbz string
		if *bdir != "" { // build pbz from human-friendly dir
			// minimal: read schema.yaml/meta.json, compute checksum, write to tmp
			data, err := os.ReadFile(*bdir+"/schema.yaml"); if err != nil { die(err) }
			...
			chk := sha256.Sum256(data); if m.Checksum == "" { m.Checksum = fmt.Sprintf("sha256:%x", chk[:]) }
			f := &bundle.File{Meta: m, SchemaYAML: data, Overlays: map[string][]byte{}, Seeds: map[string][]byte{}}
